		...
		%>
				<tr>
					...
					<td>
						...
						%>	
								<a href='/sakila/db0327/updateCustomerActive.jsp?customerId=<%=rs.getInt("customerId")%>&inventoryId=<%=inventoryId%>'>
									휴면상태해지하기<!-- customer.active 0을 1로 변경 -->
								</a>	
						<%
							} else {
						%>
								<a href='/sakila/db0327/insertRentalForm.jsp?customerId=<%=rs.getInt("customerId")%>&inventoryId=<%=inventoryId%>'>
									선택
								</a>
						<%		
							}
